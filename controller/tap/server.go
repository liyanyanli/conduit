...
func (s *server) TapByResource(req *public.TapByResourceRequest, stream pb.Tap_TapByResourceServer) error {
	if req == nil {
		return errors.New("TapByResource received nil TapByResourceRequest")
	}
	if req.Target == nil {
		return fmt.Errorf("TapByResource received nil target ResourceSelection: %+v", *req)
	}
...
func (s *server) getPodsFor(res public.ResourceSelection) ([]*apiv1.Pod, error) {
	...
	default:
		err = fmt.Errorf("Unimplemented resource type: %v", res.Resource.Type)
	}
